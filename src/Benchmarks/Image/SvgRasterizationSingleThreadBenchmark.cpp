...
namespace Elpida
{
	...
	std::vector<std::unique_ptr<Task>> SvgRasterizationSingleThreadBenchmark::GetTasks(
			const std::vector<std::reference_wrapper<const ProcessingUnitNode>>& targetProcessors,
			const std::vector<TaskConfiguration>& configuration, const EnvironmentInfo& environmentInfo) const
	{
