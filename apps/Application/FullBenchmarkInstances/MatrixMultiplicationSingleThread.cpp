...
#include "MatrixMultiplicationSingleThread.hpp"
...
{
	const Score BaseScore = 83.66 * std::mega::num;

	std::string MatrixMultiplicationSingleThread::GetName() const
	{
		return "Matrix Multiplication (Single Thread)";
	}

	std::string MatrixMultiplicationSingleThread::GetUuid() const
	{
		return "56834364-d449-4d3e-bc50-197c697c6c6f";
	}

	FullBenchmarkInstanceResult MatrixMultiplicationSingleThread::Run()
	{
	...
	MatrixMultiplicationSingleThread::MatrixMultiplicationSingleThread(const BenchmarkModel& benchmark,
