...
namespace Elpida::Application
{
	...
	BenchmarkStatistics BenchmarkStatisticsService::CalculateStatistics(
			const std::vector<BenchmarkResultModel>& results) const
	{
		double mean = 0.0;
		...
		for (auto& x : results)
		{
			deviation += std::pow(x.GetScore() - mean, 2.0);
		}
